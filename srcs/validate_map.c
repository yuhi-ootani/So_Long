...
/*   Updated: 2025/01/13 17:04:59 by oyuhi            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	is_rectangular(t_map *map)
{
	...
	while (i < map->height)
	{
		if (map->width != (int)ft_strlen(map->grid[i]))
			return (0);
		i++;
	}
	return (1);
...
int	count_exits_collects(char **map, int x, int y, t_map *m)
{
	...
	if (x < 0 || y < 0 || x >= m->height || x >= m->width || map[x][y] == '1')
		return (0);
	count = 0;
...
int	validate_path(t_map *map)
{
	...
	while (--start_y >= 0)
	{
		...
		while (--start_x >= 0)
		{
			if (copied_map[start_x][start_y] == 'P')
				break ;
		}
		if (start_x >= 0 && copied_map[start_x][start_y] == 'P')
			break ;
	}
